...
export const getCompare = (time1, time2) => {
  ...
  const [hours1, minutes1, ampm1] = time1
    .match(/(\d+):(\d+)\s([APMapm]{2})/)
    .slice(1);
  const [hours2, minutes2, ampm2] = time2
    .match(/(\d+):(\d+)\s([APMapm]{2})/)
    .slice(1);

  const militaryHours1 =
    ampm1.toUpperCase() === 'PM'
      ? parseInt(hours1, 10) + 12
      : parseInt(hours1, 10);
  const militaryHours2 =
    ampm2.toUpperCase() === 'PM'
      ? parseInt(hours2, 10) + 12
      : parseInt(hours2, 10);
